...
function JSONedtr(data, outputElement, config = {}){
    ...
    JSONedtr.getConfigValue = function (key) {
        if (
    ...
    JSONedtr.level = function (node, lvl = 0, isArrayParam ) {
        ...
        var lblKey = JSONedtr.getConfigValue('labelKey');
        var lblValue = JSONedtr.getConfigValue('labelValue');
...
//console.log('key', key, 'value', value, 'type', type);
            if (type === 'object') { /* Object || Array */
                ...
                output += '<div class="jse--row ' + rowClass + '" id="jse--row-' + JSONedtr.i + '"><input type="text" class="jse--key" data-level="' + lvl + '" value="' + key + '" placeholder="'+lblKey+'" /> : <span class="jse--typeof" data-type="' + type + '">(' + type + ')</span>';
                ...
                output += '<div class="jse--row" id="jse--row-' + JSONedtr.i + '"><input type="text" class="jse--key" data-level="' + lvl + '" value="' + key + '" '+readonly+' placeholder="'+lblKey+'" /> : <span class="jse--typeof" data-type="' + type + '">(' + type + ')</span><input type="' + inputValueType + '" class="jse--value" value="' + value + '" ' + checked + ' placeholder="'+lblValue+'" /><div class="jse--delete">✖</div></div>';
            }
        })
...
//console.log(plusO);  
        ...
        var lblSave = JSONedtr.getConfigValue('labelSave');
        ...
        if (!objectAdditionDisabled) {
            typeofHTML += '<option value="object">Object</option>';
        }
        if (!arrayAdditionDisabled) {
            typeofHTML += '<option value="array">Array</option>';
        }
        typeofHTML += '</select>';
    ...
    JSONedtr.addRow = function (row) {
        ...
        $(row).find('span.jse--typeof').html('(' + typeOf + ')').attr('data-type', typeOf);
    ...
    JSONedtr.reset = function () {
        var lblClearAll = JSONedtr.getConfigValue('labelClearAll');
        var cRet = confirm(lblClearAll);
        if ( 
            cRet &&
            JSONedtr.outputElement 
        ) {
            JSONedtr.init('{}', JSONedtr.outputElement);
        }
        return false;
    }
    
    JSONedtr.default = function () {
        var lblDefault = JSONedtr.getConfigValue('labelDefault');
        ...
        if ( 
            ...
            JSONedtr.init( JSONedtr.saveData, JSONedtr.outputElement );
        }
        return false;
    }
    ...
    JSONedtr.init = function (data, outputElement) {
        ...
        var lblClearAll = JSONedtr.getConfigValue('labelClearAll');
        var lblDefault = JSONedtr.getConfigValue('labelDefault');
        html += '<div class="jse--row"><button class="jse--default">'+lblDefault+'</button> <button class="jse--reset">'+lblClearAll+'</button></div>';
